...
let config, sections, flagsModel, raceTimeOutId
let race = {
  idleSelection: 'Local Time',
  length: 0,
  lapsLeft: 0,
  pauseTimer: true,
}
...
io.on('connection', (socket) => {
  ...
  socket.on('disconnect', () => {
    console.log(
      new Date(),
      `server connection disconnected... id: ${socket.id}`
    )
    ...
    sections.forEach((el) => {
      ...
      if (idx > -1) {
        ...
        if (el.clients.length == 0) {
          showToast(
            `alla skärmar för sektion ${el.section} bortkopplade`,
            'red'
          )
          el.active = false
          el.flags = []
  ...
  socket.on(
    'selectFlag',
    ({ clickedFlag, blink, number, allScreens, mainScreen }) => {
      let flagAttributes = flagsModel.find((flag) => flag.name == clickedFlag)
      let selectedScreen = sections.find((el) => el.active)
      let allActiveScreens = sections.filter(
        (screen) => screen.clients.length > 0
      )
  ...
  socket.on('clickSection', ({ section }) => {
    ...
    if (!isOnline) {
      showToast(
        `Ingen skärm uppkopplad för sektion ${clickedSection.section}`,
        'orange'
      )
      return
    }
  ...
  socket.on('clickIdleSelection', ({ btn }) => {
    race.idleSelection = btn
    io.emit('selectIdle', race.idleSelection)
  })

  socket.on('pauseRaceTimer', (isPause) => {
    race.pauseTimer = !race.pauseTimer
    if (isPause !== null) race.pauseTimer = isPause
  })

  socket.on('decreaseLapsLeft', () => {
    if (race.lapsLeft > 0) race.lapsLeft--
    updateClient()
  })

  socket.on('clickStart', ({ btn, text, time, length, laps }) => {
    io.emit('clickStart', {
      btn: btn,
      text: text,
      time: time,
...
function updateClient(options) {
  io.emit('updateClient', { sections, config, options, race })
}
...
function raceTimeKeeper() {
  raceTimeOutId = setTimeout(() => {
    if (race.length > 0) raceTimeKeeper()
    let HH = Math.floor(race.length / 3600)
    let mm = String(Math.floor((race.length / 60) % 60)).padStart(2, '0')
    let ss = String(race.length % 60).padStart(2, '0')
    // console.log(`${HH}:${mm}:${ss}`)
    io.emit('updateRaceTime', `${HH}:${mm}:${ss}`)
    if (!race.pauseTimer) race.length--
  }, 1000)
}
...
function handleSelectFlag(clickedFlag, blink, number, selectedScreen) {
  ...
  let flagIndex =
    selectedScreen &&
    selectedScreen.flags.findIndex((flag) => {
  ...
  if (flagAttributes.prio > getLowestPrio()) {
    showToast(
      `Flagga med prio ${getLowestPrio()} visas, släck den först`,
      'red'
    )
    return
  }
  ...
  function isFlagSelected() {
    return selectedScreen.flags.find(
      (flag) => flag.name == clickedFlag && flag.number == number
    )
  }
